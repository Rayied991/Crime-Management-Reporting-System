...
export class RegistrationDTO {
  ...
  @Matches(/^[a-zA-Z]+$/, {
    message: 'First Name should not contain numbers and symbol',
  })
  fname: string;    
  ...
  @Matches(/^[a-zA-Z]+$/, {
    message: 'Last Name should not contain numbers and symbol',
  })
  lname: string;
  ...
  @Matches(/^[a-zA-Z]+$/, {
    message: 'Location name should not contain numbers and symbol',
  })
  location: string;
  ...
  @IsNumber(
    {},
    { message: 'Phone numbers must be numbers,dont use cherecters' },
  )
  ...
  @Matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/, {
    message:
      'Password must be more than 8 letters and upper case and lower case Format with at least one numerical character',
  })
  password: string;
...
export class Add_Wanted_List_DTO {
  criminal_name: string;
  criminal_email: string;
  criminal_phonenumber: string;
  wanted_criminal_no: string;
