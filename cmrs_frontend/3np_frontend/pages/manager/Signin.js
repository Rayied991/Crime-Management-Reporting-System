...
export default function LoginPage() {
  ...
  const [password, setPassword] = useState("");
    ...
    const onSubmit = async (e) => {
      ...
      try {
        const response = await axios.post( process.env.NEXT_PUBLIC_API_ENDPOINT+"/manager/signin/", {ManagerID, password })
        console.log("res: "+response.data)
        
          sessionStorage.setItem('ManagerID', response.data);
          document.cookie = `${response.data}; domain=localhost; path=/`;
          login(ManagerID, document.cookie);
          console.warn("ManagerID: "+ ManagerID)
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            <div class='text-center dark:text-white'>
                <h1>Signin Page</h1>
                    ...
                    <div>
                        <label htmlFor="ManagerID" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your ManagerID</label>
                        <input type="number" name="ManagerID" id="ManagerID" value={ManagerID} 
                        onChange={(e) => setManagerID(e.target.value)} class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter your Id" required=""
                        {...register("ManagerID", { required: true })}/>
                        {errors.ManagerID &&  <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> This Field is Required!</p>}
                    </div>
                    <div>
...
<p class="text-sm font-light text-gray-500 dark:text-gray-400">
            ...
            <a href="/Manager/Signup" class="font-medium text-primary-600 hover:underline dark:text-primary-500 hover:text-blue-500">
                Sign up
            </a>
        </p>
