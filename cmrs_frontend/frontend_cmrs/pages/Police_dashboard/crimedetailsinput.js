...
export default function AdminInputForm() {
  ...
  const [wantedlist2, setinputAdminid] = useState();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!wantedlist2) {
      ...
      console.info(wantedlist2);
  
    try {
      const response = await axios.get(
        `http://localhost:3000/police/scrimedetails/${wantedlist2}`
      );
      ...
      if (response.data) {
        ...
        router.push(`/Police_dashboard/viewcrimedetails/${wantedlist2}`);
      } else {
