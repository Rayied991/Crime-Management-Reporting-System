...
export default function AdminInputForm() {
  ...
  const [PostId, setinputAdminid] = useState();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/police/scrimestatus/${PostId}`
      );
      ...
      if (response.data) {
        ...
        router.push(`/Police_dashboard/viewcrimestatus/${PostId}`);
      } else {
  ...
  return (
    <>
     <_Title title= "Search Crime Status No" />
     <Layout >
...
<section class="bg-gray-50 dark:bg-gray-900  bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/conference.jpg')] bg-gray-700 bg-blend-multiply">
    ...
    <form onSubmit={handleSubmit}>
      <label class="block mb-2 text-sm font-medium text-gray-900 text-white">
        Enter Any Criminal PostId Number:
        <input
          type="number"
          value={PostId}
